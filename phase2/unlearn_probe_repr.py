"""
Probe-guided representation unlearning for Evo-1-8k-base on the merged Phase 2
selective-unlearning objective.

Per step:
  L_forget = squared standardized probe component on positive target batches
  L_retain = representation MSE (+ optional cosine penalty) to a frozen
             reference model on retain batches
  loss     = alpha_forget * L_forget + alpha_retain * L_retain

There is no cross-entropy term anywhere: this method never touches the language
modelling objective directly, it only moves pooled representations onto the
fixed probe decision boundary while anchoring retain representations.
...
HISTORY -- read this before comparing against archived results.

  This file used to be `phase2/unlearn_gd.py`, and for part of the project's
  history that filename contained classic gradient difference instead:

      loss = -alpha_forget * CE(forget) + alpha_retain * CE(retain)

  The objective was replaced with the representation objective above while the
  filename, the sweep method key `gd`, and the metadata value
  `method: gradient_difference` were all kept, so nothing signalled the change.
  Classic gradient difference has since been restored to
  `phase2/unlearn_gd.py`, and this file was renamed to match what it actually
  computes. It reports `method: probe_guided_representation`.

  Every archived GD run with a committed meta.json (`lora_gd_*`, `gd_full_ar5`)
  came from the CE objective, so use `phase2/unlearn_gd.py` to reproduce those.
  The `refseq_gd_projinit_*` sweep entries came from THIS objective and now
  select `method: probe_repr` in phase2/sweep_configs/task2_sweeps.json.

RELATED -- `phase2/unlearn_probe.py` implements the same forget loss as its
`--forget-objective component_zero` branch. It differs by tying the trainable
layers to the probe target layers, and by offering `output_kl`/`ce` retain terms
instead of the cosine penalty here. The two are candidates for consolidation;
see README.md, Adding your own method.
"""
import argparse
import json
import os
...
def build_probe_repr_metadata(
    *,
    ...
    extra = {
        "method": "probe_guided_representation",
...
def main() -> None:
    if read_manifest is None:
        raise ImportError("phase1.utils.read_manifest is required to run unlearn_probe_repr.py")
    ...
    print(f"[probe-repr] condition={args.condition} steps={args.steps} lr={args.lr}")
    print("[probe-repr] loss=probe_guided_representation")
    save_steps = {step for step in parse_save_steps(args.save_steps) if 1 <= step <= args.steps}
    run_name = args.run_name if args.run_name else f"probe_repr_{args.condition}"
    run_dir = os.path.join(args.out_dir, run_name)
    os.makedirs(run_dir, exist_ok=True)
    init_ckpt, init_source = resolve_init_ckpt(args)
    if save_steps:
        print(f"[probe-repr] intermediate save steps: {sorted(save_steps)}")
    ...
    print(f"[probe-repr] trainable layers: {layers}")
    print(f"[probe-repr] probe loss layers: {loss_layers}")
    print(f"[probe-repr] trainable params: {count_trainable(model):,}")
    ...
    for spec in target_specs:
        ...
        print(
            f"[probe-repr] target={spec['name']} layers={spec['layers']} "
            f"forget_train={len(spec['train_records'])}"
        )
    print(f"[probe-repr] retain train={len(retain)}")
    ...
    pbar = tqdm(range(args.steps), desc=f"probe-repr-{args.condition}")
    t0 = time.time()
    for step in pbar:
    ...
    print(f"[probe-repr] done in {elapsed:.1f}s")
    ...
    model_capture.close()
    ref_capture.close()
    print(f"[probe-repr] saved to {run_dir}")
